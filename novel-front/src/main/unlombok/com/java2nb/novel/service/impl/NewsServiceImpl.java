...
public class NewsServiceImpl implements NewsService {
    ...
    public List<News> listIndexNews() {
        ...
        if (result == null || result.size() == 0) {
            SelectStatementProvider selectStatement = select(id, catName, catId, title, createTime).from(news).orderBy(createTime.descending()).limit(2).build().render(RenderingStrategies.MYBATIS3);
    ...
    public News queryNewsInfo(Long newsId) {
        SelectStatementProvider selectStatement = select(news.allColumns()).from(news).where(id, isEqualTo(newsId)).build().render(RenderingStrategies.MYBATIS3);
    ...
    public PageBean<News> listByPage(int page, int pageSize) {
        ...
        SelectStatementProvider selectStatement = select(id, catName, catId, title, createTime).from(news).orderBy(createTime.descending()).build().render(RenderingStrategies.MYBATIS3);
    ...
    public NewsServiceImpl(final FrontNewsMapper newsMapper, final CacheService cacheService) {
